  ...
  useEffect(() => {
    ...
    const loadDevices = async () => {
      try {
        ...
        // Now enumerate devices - wait a bit for device labels to populate
        ...
        // 1. Previously selected device (if still available)
        // 2. External/USB device (preferred for better quality)
        ...
        if (audioInputs.length > 0) {
          setSelectedDeviceId(currentId => {
            ...
            if (!currentDeviceStillAvailable || !currentId) {
              // Try to find external device (better quality, user intent to use it)
    ...
    // Listen for device changes (e.g., plugging/unplugging external mic)
