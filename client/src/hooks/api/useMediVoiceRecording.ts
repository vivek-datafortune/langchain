...
import { playStartListeningSound, playStopListeningSound } from '../../lib/utils/listeningSounds'
...
export function useMediVoiceRecording(deviceId?: string) {
  ...
  const sendToEnquiry = useCallback(async (chunks: Blob[]) => {
    ...
    await enquireAudio(blob, {
      ...
      onResult: (r) => {
        console.log('[useMediVoiceRecording] audio enquiry result:', r, '| conversationMessageCount:', r?.conversationMessageCount)
        setEnquiryResult(r)
        setStageInfo(null)
        setPhase('idle')
      },
  ...
  const submitTextQuery = useCallback(async (text: string) => {
    ...
    await enquireText(trimmed, {
      onStage: (s) => setStageInfo(s),
      onResult: (r) => {
        console.log('[useMediVoiceRecording] text enquiry result:', r, '| conversationMessageCount:', r?.conversationMessageCount)
        setEnquiryResult(r)
        setStageInfo(null)
        setPhase('idle')
      },
